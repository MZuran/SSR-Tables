 ...
 */
export function binaryArray(binary) {
...
/**
 * Converts a binary string or binary array to a hexadecimal string with "0x" prefix,
 * ensuring the result has at least two hex digits (e.g., "0x02" instead of "0x2").
 * 
 * @param {string|number[]|number} input - Binary string (e.g., "00001010"), array (e.g., [0,0,0,0,1,0,1,0]), or number (e.g., 1010).
 * @returns {string} Hexadecimal representation with "0x" prefix and at least two digits (e.g., "0x0A").
 */
export function binaryToHex(input) {
    let binStr = "";
    ...
    const hex = parseInt(binStr, 2).toString(16).toUpperCase().padStart(2, '0');
    return `0x${hex}`;
}
